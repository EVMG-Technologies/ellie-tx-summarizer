...
export function Summarizer() {
  ...
  const runSummary = (sig: string, presetSummary?: string) => {
    ...
    setLoading(true);
    setSummary(null);
    const delay = 2800 + Math.random() * 900;
    window.setTimeout(() => {
      const text =
        presetSummary ??
        `Transaction ${truncate(sig)} transferred 1.25 SOL from Wallet A to Wallet B, interacted with the Jupiter aggregator program, and paid a network fee of 0.000005 SOL. The transaction confirmed successfully on the Solana mainnet.`;
      setSummary(text);
      setSummarySig(sig);
      setLoading(false);
    }, delay);
